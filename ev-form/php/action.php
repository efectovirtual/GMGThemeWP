...
/************************************************/
	...
	$your_email = "info@servidorevs.com";

	/* Your name or your company name goes here */
	$your_name = "EfectoVirtual";

	/* Message subject */
	$your_subject = "GMG Apply";
...
/************************************************/
	...
	$countryphone  	 = htmlspecialchars($countryphone, ENT_QUOTES, 'UTF-8');
	...
	$countryphone  	 = substr($countryphone, 0, 70);
...
/************************************************/
	...
	/* Country phone */
	if ($validate_countryphone){
		$result = validateCountryphone($countryphone);
		if ($result !== "valid") {
			$error_text[] = $result;
		}
	}
...
/************************************************/
	if ($send_letter) {
		...
		if ($smtp) {
			...
			$mail->Host = "mail.servidorevs.com";		// Specify main and backup server
			$mail->SMTPAuth = true;										// Enable SMTP authentication
			$mail->Username = "info@servidorevs.com";							// SMTP username
			$mail->Password = "artevirtual00";							// SMTP password
		...
		$recipients = false;
		if ($recipients) {
			$recipients = array("info@servidorevs.com" => "name of recipient",
